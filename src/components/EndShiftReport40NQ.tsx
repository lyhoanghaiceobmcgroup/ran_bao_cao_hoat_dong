...
const sendTelegramMessage40NQ = async (message: string): Promise<boolean> => {
  try {
    ...
    console.error('Error sending message to HN40 Telegram:', error);
    return false;
  }
};

const sendTelegramPhoto40NQ = async (photo: File, caption?: string): Promise<boolean> => {
  try {
    ...
    console.error('Error sending photo to HN40 Telegram:', error);
    return false;
  }
};
...
export default function EndShiftReport40NQ() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const reportData: EndShiftReportData = {
        // Thông tin chung - hardcoded for HN40
        storeId: HN40_STORE_ID,
        branchName: HN40_BRANCH_NAME,
      ...
      // Send to HN40 Telegram
      const success = await sendEndShiftReportToTelegram40NQ(reportData);
      
      if (success) {
        setSubmitStatus('success');
        setSubmitMessage('Báo cáo ra ca đã được gửi thành công đến Telegram HN40!');
        // Reset form after successful submission
        setTimeout(() => {
          (e.target as HTMLFormElement).reset();
          setSubmitStatus('idle');
          setSubmitMessage('');
          // Navigate back to HN40 dashboard
          navigate('/HN40-dashboard');
        }, 3000);
      } else {
        throw new Error('Failed to send report to HN40 Telegram');
      }
    } catch (error) {
      console.error('Error submitting HN40 report:', error);
      setSubmitStatus('error');
      setSubmitMessage('Có lỗi xảy ra khi gửi báo cáo HN40. Vui lòng thử lại.');
    } finally {
      setIsSubmitting(false);
    }
  };

  const sendEndShiftReportToTelegram40NQ = async (data: EndShiftReportData): Promise<boolean> => {
    try {
      // Format message for HN40
      const message = `🔴 <b>BÁO CÁO RA CA - ${data.branchName}</b>\n\n` +
        `📍 <b>Cửa hàng:</b> ${data.storeId}\n` +
        `⏰ <b>Ca:</b> ${data.shiftPeriod}\n` +
        `👤 <b>Quản lý:</b> ${data.manager}\n` +
        `🕐 <b>Giờ ra ca:</b> ${data.endTime}\n\n` +
        
        `💰 <b>DOANH THU POS:</b>\n` +
        `• Tiền mặt: ${data.pos_cash.toLocaleString('vi-VN')} VND\n` +
        `• Thẻ: ${data.pos_card.toLocaleString('vi-VN')} VND\n` +
        `• QR/E-Wallet: ${data.pos_ewallet.toLocaleString('vi-VN')} VND\n` +
        `• <b>Tổng: ${data.pos_total.toLocaleString('vi-VN')} VND</b>\n` +
        `• Số bill: ${data.pos_bills}\n` +
        `• Số khách: ${data.pos_guests}\n` +
        `• AOV: ${data.pos_aov.toLocaleString('vi-VN')} VND\n\n` +
        
        `🧾 <b>ĐỐI SOÁT QUỸ:</b>\n` +
        `• Lý thuyết: ${data.fund_theory.toLocaleString('vi-VN')} VND\n` +
        `• Thực đếm: ${data.fund_counted.toLocaleString('vi-VN')} VND\n` +
        `• Chênh lệch: ${data.fund_delta.toLocaleString('vi-VN')} VND\n\n`;

      // Send main message
      const messageSuccess = await sendTelegramMessage40NQ(message);
      
      // Send additional sections if they have data
      if (data.inventory.length > 0) {
        const inventoryMsg = `📦 <b>KHO & NGUYÊN LIỆU:</b>\n` +
          data.inventory.map(item => 
            `• ${item.name}: Tồn cuối ${item.last}, Lý thuyết ${item.theo}, Chênh ${item.diff}${item.note ? ` (${item.note})` : ''}`
          ).join('\n');
        await sendTelegramMessage40NQ(inventoryMsg);
      }
      
      if (data.staff.length > 0) {
        const staffMsg = `👥 <b>NHÂN SỰ CA:</b>\n` +
          data.staff.map(person => 
            `• ${person.name} (${person.role}): Ra ca ${person.timeOut}, ${person.hours}h, Tip ${person.tip}${person.note ? ` - ${person.note}` : ''}`
          ).join('\n');
        await sendTelegramMessage40NQ(staffMsg);
      }
      
      if (data.mkt_feedback || data.mkt_voucherCount > 0) {
        const marketingMsg = `📈 <b>MARKETING & KHÁCH HÀNG:</b>\n` +
          `• Voucher: ${data.mkt_voucherCount} (${data.mkt_voucherValue.toLocaleString('vi-VN')} VND)\n` +
          `• Khách mới: ${data.mkt_newCustomers}, Quay lại: ${data.mkt_returningCustomers}\n` +
          `• Rating: ${data.mkt_rating}/5, NPS: ${data.mkt_nps}\n` +
          (data.mkt_feedback ? `• Feedback: ${data.mkt_feedback}` : '');
        await sendTelegramMessage40NQ(marketingMsg);
      }
      
      if (data.incidents.length > 0) {
        const incidentMsg = `⚠️ <b>SỰ CỐ PHÁT SINH:</b>\n` +
          data.incidents.map(incident => 
            `• ${incident.name} (${incident.status}): ${incident.desc} → ${incident.action}`
          ).join('\n');
        await sendTelegramMessage40NQ(incidentMsg);
      }
      ...
      if (data.images && data.images.length > 0) {
        for (const image of data.images) {
          ...
          try {
            await sendTelegramPhoto40NQ(image, data.imageNotes);
          } catch (error) {
      ...
      console.error('Error sending HN40 end shift report:', error);
      return false;
    }
  };
            ...
            <section className="border border-border rounded-xl p-4 bg-card shadow-card">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <input name="storeId" value={HN40_STORE_ID} readOnly className="w-full p-2 border border-input rounded-md bg-muted" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-2">Chi nhánh (branchName)</label>
                  <input name="branchName" value={HN40_BRANCH_NAME} readOnly className="w-full p-2 border border-input rounded-md bg-muted" />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input name="accountId" placeholder="VD: staff-HN40" className="w-full p-2 border border-input rounded-md bg-background" />
                </div>
                <div>
