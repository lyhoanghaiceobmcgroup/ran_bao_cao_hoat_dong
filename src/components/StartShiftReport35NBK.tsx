...
const sendTelegramPhoto35NBK = async (file: File, caption: string): Promise<boolean> => {
  try {
    ...
    const response = await fetch(`https://api.telegram.org/bot${HN35_BOT_TOKEN}/sendPhoto`, {
      method: 'POST',
      body: formData
    });
...
export default function StartShiftReport35NBK() {
...
• Chuẩn bị ca làm việc: ${checklistItems.preparation ? '✅' : '❌'}`;
      ...
      const message = `🏪 <b>BÁO CÁO VÀO CA - ${HN35_STORE_ID}</b>\n\n` +
        `📋 <b>THÔNG TIN CHUNG</b>\n` +
        `• Cửa hàng: ${HN35_STORE_ID}\n` +
        `• Chi nhánh: ${HN35_BRANCH_NAME}\n` +
      ...
      // Send text message using HN35-specific function
      const success = await sendTelegramMessage35NBK(message);
      ...
      if (billImagesInput && billImagesInput.files && billImagesInput.files.length > 0) {
        ...
        for (let i = 0; i < Math.min(billImagesInput.files.length, 6); i++) {
          ...
          const photoSuccess = await sendTelegramPhoto35NBK(file, caption);
          if (!photoSuccess) {
      ...
      setStatusMessage('✅ Xác nhận gửi thành công! Báo cáo đã được gửi đến Telegram HN35.');
        ...
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Button variant="ghost" size="sm" onClick={() => navigate('/HN35-dashboard')}>
            ...
            <form id="startShiftForm" className="flex flex-col gap-4">
              ...
              <section className="p-4 border border-border rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button type="button" onClick={addStaff} size="sm" variant="outline">
                ...
                <div className="space-y-3">
                  {staffList.map((staff, index) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-5 gap-2 p-3 border border-border/50 rounded-lg">
                      <input 
              ...
              <section className="p-4 border border-border rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button type="button" onClick={addInventory} size="sm" variant="outline">
                    <Plus className="h-4 w-4 mr-1" />
                    Thêm
                  </Button>
                </div>
                <div className="space-y-3">
                  {inventoryList.map((item, index) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-7 gap-2 p-3 border border-border/50 rounded-lg">
                      <input 
                        placeholder="Tên mặt hàng"
                        value={item.item}
                        onChange={(e) => updateInventory(index, 'item', e.target.value)}
                        className="px-2 py-1 border border-border rounded text-sm focus:outline-none focus:ring-1 focus:ring-ring md:col-span-2"
                      />
                      <input 
                        placeholder="Tồn kho"
                        value={item.onHand}
              ...
              <section className="p-4 border border-border rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <label className="flex flex-col gap-2">
                    ...
                    <input 
                      name="promotions" 
                      placeholder="VD: Giảm 20% combo, mua 2 tặng 1..."
                      className="px-3 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                  </label>
                  <label className="flex flex-col gap-2 md:col-span-2">
                    ...
                    <textarea 
                      name="marketingPlan" 
                      rows={2}
                      placeholder="VD: Post story Instagram, chạy ads Facebook, tặng voucher..."
                      className="px-3 py-2 border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-ring"
                    />
                  </label>
                </div>
              ...
              <section className="p-4 border border-border rounded-lg">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <Button type="button" onClick={addIncident} size="sm" variant="outline">
                    <Plus className="h-4 w-4 mr-1" />
                    Thêm
                  </Button>
                </div>
                <div className="space-y-3">
                  {incidentList.map((incident, index) => (
                    <div key={index} className="grid grid-cols-1 md:grid-cols-6 gap-2 p-3 border border-border/50 rounded-lg">
                      <select 
              ...
              <section className="p-4 border border-border rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <label className="flex flex-col gap-2">
                    <span className="text-sm font-medium">Doanh thu mục tiêu (VND)</span>
                    <input 
