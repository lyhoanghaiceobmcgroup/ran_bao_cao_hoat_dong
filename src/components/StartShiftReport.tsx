...
import { sendTelegramMessage, sendTelegramPhoto } from '@/utils/telegramApi';
...
export default function StartShiftReport() {
  ...
  const getStoreId = () => {
    ...
    return 'RAN-35NBKHN'; // Default to HN35
  };
...
• Chuẩn bị ca làm việc: ${checklistItems.preparation ? '✅' : '❌'}`;
      ...
      // Send text message using telegramApi
      ...
      if (billImagesInput && billImagesInput.files && billImagesInput.files.length > 0) {
        ...
        for (let i = 0; i < Math.min(billImagesInput.files.length, 6); i++) {
          ...
          const photoSuccess = await sendTelegramPhoto(file, caption);
          if (!photoSuccess) {
